...
export const getWeatherUpdate = async (lat: number, lon: number): Promise<string | null> => {
    try {
        const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code&temperature_unit=celsius`);
...
export const getTodaysHoliday = async (): Promise<string | null> => {
    try {
        ...
        if (response.status === 200) {
            const holidaysResponse = await fetch(`https://date.nager.at/api/v3/PublicHolidays/${year}/${countryCode}`);
...
export const generateCoolShowName = async (): Promise<string> => {
    try {
        const response = await fetch('https://random-data-api.com/api/device/random_device');
        const data = await response.json();
